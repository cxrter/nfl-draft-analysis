...
text.append("textPath")
  ...
  player_name = "<div class='row'><div class='col text-center'><p class='lead'>" + d['player'] + "</p></div></div>"
  player_img = "<div class='row'><div class='col text-center'><img src='" + d.img + "' alt='../static/no-img.png' width='100px'></div></div>"
  player_info  = player_name.concat(player_img)

  sum_stats = ['G','AV','QBrec','Cmp%','Yds','Y/A','TD','Int','FantPt','Sk','Solo','FF','GS','Rush','Rec','Y/R','Ret','Rt','FGM','FGA','XPM','XPA']
  sum_stats_map = ['Games Played', 'Accumulated Value', 'Record', 'Completion %', 'Yards', 'Yards per Attempt', 'Touchdowns', 'Interceptions Thrown',
                    'Fantasy Points', 'Sacks', 'Tackles', 'Forced Fumbles', 'Games Started', 'Rushing Attempts', 'Receptions', 'Yards per Reception',
                    'Punts Returned', 'Field Goals Made', 'Field Goal Attempts', 'Extra Points Made', 'Extra Points Attempted']
  stats = ''
  value = ''
  for (i in sum_stats){
  stat_data = sum_stats[i]
  stat = sum_stats_map[i]
  if (d[stat_data]){
  str = "<div class='row'><div class='col-auto text-left'><p class='lead' style='margin:0px; font-size:1vw'>" + stat + ": </p></div><div class='col'><p class='lead text-right' style='margin:0px; font-size:1vw'><strong>" + d[stat_data] + "</strong></p></div></div>"
  stats = stats.concat(str)
  card = player_info.concat(stats)
  }}
...
color_trio = ['DB222A', 'F9C80E', '3F88C5', '7AC74F', 'BD93BD']
...
var upper_year = $('#slider-range').slider( "values", 1 );
var lower_year = $('#slider-range').slider( "values", 0 );
...
var circles = scatter_svg.selectAll("circle")
    ...
    circles
      .attr("class", function (d) { return "dot " + d.pos + " " + d.modern_code + " " + d.player.split(' ').join('')} )
    ...
    circles.enter()
        .append("circle")
      .attr("class", function (d) { return "dot " + d.pos + " " + d.modern_code + " " + d.pfr_player_code} )
      .attr("cx", function (d) { return x(d.draft_pick); } )
      .attr("cy", function (d) { return y(d.draft_av/d.years_with_team); } )
      .attr("r", 4)
      .style("stroke-width", 1)
...
function update_highlight() {
      ...
      d3.selectAll(".team_checkbox").each(function(d){
        ...
        if(cb.property("checked")){
          ...
          scatter_svg.selectAll("." + CSS.escape(team))
          .raise()
          .attr("r", 4)
          .style("stroke-width", 0)
          .style('opacity', 1)
          .style("fill", color_trio[(n_checked-1)%5])
          d3.select(".btn-"+team)
            .style('background-color', '#' + color_trio[(n_checked-1)%5])
          }

         // Otherwise mute it
        else{
        scatter_svg.selectAll("." + CSS.escape(team)).attr("r", 4).style("fill", '#e1e1e1').style('opacity', 0.1).style("stroke-width", 1).style("stroke", 'grey');
...
//<style>.btn-"+allTeams[i]+".active {background-color: "+get_color(allTeams[i])+" !important;}</style>
...
d3.selectAll("#slider-range").on('click', function(d){
update_chart()
console.log('ch')
})
